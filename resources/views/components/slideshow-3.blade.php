...
@for ($i = 0; $i < count($banners); $i++)
    ...
    <div class="h-56 w-full sm:h-72 md:h-96 lg:w-full flex justify-center items-center carousel-item absolute opacity-0 bg-center bg-cover" style="height:500px; background-image: url({{ asset('storage/'.$banners[$i] ) }})">
    ...
    @if (count($banners)>1)
        @if ($i === 0)
            <label for="carousel-{{count($banners)}}"
                class="control-{{$i+1}}  w-10 h-10 ml-2 md:ml-10 absolute cursor-pointer hidden font-bold text-black hover:text-white rounded-full bg-white hover:bg-rose-400 leading-tight text-center z-10 inset-y-0 left-0 my-auto flex justify-center content-center"><i
                class="fas fa-angle-left mt-3"></i></label>
            
            <label for="carousel-{{$i+2}}"
                class="next control-{{$i+1}}  w-10 h-10 mr-2 md:mr-10 absolute cursor-pointer hidden font-bold text-black hover:text-white rounded-full bg-white hover:bg-rose-400 leading-tight text-center z-10 inset-y-0 right-0 my-auto"><i
    ...
    @for ($i = 0; $i < count($banners); $i++)
        <li class="inline-block mr-3">
            <label for="carousel-{{$i+1}}"
                class="carousel-bullet cursor-pointer block text-4xl text-white hover:text-rose-400">•</label>
        </li>
    @endfor
    {{-- <li class="inline-block mr-3">
